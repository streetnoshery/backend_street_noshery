...
import {
  ...
  REVIEWS,
  SHOPEMAILS,
} from './database-provider.constants';
import { ConfigService } from '@nestjs/config';
import {
  ICustomer,
  customerSchema,
} from 'src/customer/model/customer-model.model';
import {
  ICustomerOtp,
  customerOtpSchema,
} from 'src/customer/model/customer-otp.model';
import { IMenu, MenuSchema } from 'src/menu/model/menu.model';
import {
  ICustomerOrderData,
  customerOrderDataSchema,
} from 'src/order/model/order.model';
...
import { emailSchema, IEmail } from 'src/notification/model/email.model';
...
export const databaseProvider = [
  {
    ...
    useFactory: (): Connection => {
      ...
      const connection = mongoose.createConnection(
        'mongodb+srv://streetnoshery:Sumit%40Godwan%401062@streetnoshery.g7ufm.mongodb.net/street_noshery?retryWrites=true&w=majority',
        {
  ...
  },
  {
    provide: EMAILS,
    useFactory: (connection: mongoose.Connection) => {
      const userEmails = connection.model<IEmail>(
        'streetnosheryuseremails',
        emailSchema,
        'user_emails',
      );
      userEmails.syncIndexes();
      return userEmails;
    },
    inject: [DATABASE_CONNECTION, ConfigService],
  },
